    ...
    def post(self, request):
        ...
        for user_data in data:
            if len(user_data) != 3 or len(user_data[0]) > 32:
                return self.error(f"데이터를 처리하는 동안 오류가 발생했습니다. '{user_data}'")
...
class GenerateUserAPI(APIView):
    ...
    def get(self, request):
        ...
        if not file_id:
            return self.error("유효하지 않음, 파일이름이 필요합니다")
        if not re.match(r"^[a-zA-Z0-9]+$", file_id):
            return self.error("잘못된 파일이름")
    ...
    def post(self, request):
        ...
        if number_max_length + len(data["prefix"]) + len(data["suffix"]) > 32:
            return self.error("사용자 이름은 32 자 이하 여야합니다.")
        if data["number_from"] > data["number_to"]:
            return self.error("시작 번호는 종료 번호보다 작아야합니다")
